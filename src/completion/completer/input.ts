...
abstract class InputAbstract implements CompletionProvider {
    ...
    private filterIgnoredFiles(uri: vscode.Uri, files: string[], baseDir: string): string[] {
        const excludeGlob = (Object.keys(vscode.workspace.getConfiguration('files', null).get('exclude') || {})).concat(vscode.workspace.getConfiguration('dev-workshop', uri).get('intellisense.file.exclude') || [] ).concat(ignoreFiles)
            ...
            try {
                ...
                files.forEach(file => {
                    ...
                    if (fs.lstatSync(filePath).isDirectory()) {
                        ...
                        const preview = vscode.workspace.getConfiguration('dev-workshop').get('intellisense.includegraphics.preview.enabled') as boolean
...
class Input extends InputAbstract {
    ...
    getBaseDir(currentFile: string, _importFromDir: string, macro: string): string[] {
        ...
        if (['includegraphics', 'includesvg'].includes(macro) && this.graphicsPath.size > 0) {
            ...
            const baseConfig = vscode.workspace.getConfiguration('dev-workshop', vscode.Uri.file(currentFile)).get('intellisense.file.base')
