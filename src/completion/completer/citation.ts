...
function provide(uri: vscode.Uri, line: string, position: vscode.Position): CompletionItem[] {
    ...
    const configuration = vscode.workspace.getConfiguration('dev-workshop', uri)
...
function browser(args?: CompletionArgs) {
    const configuration = vscode.workspace.getConfiguration('dev-workshop', args?.uri)
    const label = configuration.get('intellisense.citation.label') as string
...
function getItem(key: string, configurationScope?: vscode.ConfigurationScope): CitationItem | undefined {
    ...
    if (entry && !(entry.detail || entry.documentation)) {
        const configuration = vscode.workspace.getConfiguration('dev-workshop', configurationScope)
        const fields = readCitationFormat(configuration)
        // We need two spaces to ensure md newline
 ...
 *
 * @param file The path of a Dev file. If `undefined`, the keys of `bibEntries` are used.
...
/**
 * Returns aggregated bib entries from `.bib` files and bibitems defined on Dev files included in the root file.
 *
...
async function parseBibFile(fileName: string) {
    ...
    const configuration = vscode.workspace.getConfiguration('dev-workshop', vscode.Uri.file(fileName))
