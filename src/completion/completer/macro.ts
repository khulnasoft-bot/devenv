...
initialize()
function initialize() {
    const excludeDefault = (vscode.workspace.getConfiguration('dev-workshop').get('intellisense.package.exclude') as string[]).includes('lw-default')
    ...
    const userCmds = vscode.workspace.getConfiguration('dev-workshop').get('intellisense.command.user') as {[key: string]: string}
...
function parseContent(content: string, filePath: string): CmdEnvSuggestion[] {
    ...
    const packages = lw.completion.usepackage.getAll('dev-expl3')
    Object.entries(packages).forEach(([packageName, options]) => {
