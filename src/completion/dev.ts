    ...
    completion(type: string, args: CompletionArgs): vscode.CompletionItem[] {
        ...
        switch (type) {
            ...
            case 'environment':
                ...
                break
            case 'macro':
                reg = args.langId === 'dev-expl3' ? /\\([a-zA-Z_@]*(?::[a-zA-Z]*)?)$/ : /\\(\+?[a-zA-Z]*|(?:left|[Bb]ig{1,2}l)?[({[]?)$/
                provider = macroProvider
                break
            case 'argument':
                reg = args.langId === 'dev-expl3' ? /\\([a-zA-Z_@]*(?::[a-zA-Z]*)?)((?:\[.*?\]|{.*?})*)[[{][^[\]{}]*$/ : /\\(\+?[a-zA-Z]*)((?:\[.*?\]|{.*?})*)[[{][^[\]{}]*$/
...
export class AtProvider implements vscode.CompletionItemProvider {
    ...
    updateTrigger() {
        const configuration = vscode.workspace.getConfiguration('dev-workshop')
        const triggerCharacter = configuration.get('intellisense.atSuggestion.trigger.dev') as string
