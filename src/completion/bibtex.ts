...
export class BibProvider implements vscode.CompletionItemProvider {
    ...
    private initialize() {
        const configuration = vscode.workspace.getConfiguration('dev-workshop', this.scope)
        ...
        switch (citationBackend) {
            case 'bibtex':
                ...
                break
            case 'bibdev':
                entriesFile = `${lw.extensionRoot}/data/bibdev-entries.json`
                optEntriesFile = `${lw.extensionRoot}/data/bibdev-optional-entries.json`
                entriesReplacements = configuration.get('intellisense.bibdevJSON.replace') as {[key: string]: string[]}
                break
            default:
