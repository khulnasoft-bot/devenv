...
import type { ClientRequest } from '../../types/dev-workshop-protocol-types'
...
function toPDF(args?: {line: number, filePath: string}, forcedViewer: 'auto' | 'tabOrBrowser' | 'external' = 'auto', pdfFile?: string) {
    ...
    if (!active) {
        logger.log('No active Dev editor found or previous one recorded.')
        return
    }
    ...
    if (args === undefined) {
        ...
        if (!lw.file.hasTeXLangId(active.document.languageId)) {
            logger.log(`${filePath} is not valid Dev.`)
            return
        }
