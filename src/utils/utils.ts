...
/**
 * Strip text and comments from Dev, leaving only macros and environments.
 *
 * @param raw The raw Dev content as a string
 * @returns The stripped Dev macro barebone
 */
export function stripText(raw: string): string {
    ...
    // The following regex defines a Dev macro.
...
export function stripCommentsAndVerbatim(text: string): string {
    ...
    const verbatimEnvs = configuration.get('dev.verbatimEnvs') as string[]
...
/**
 * Return a function replacing placeholders of Dev recipes.
 *
...
export function replaceArgumentPlaceholders(rootFile: string, tmpDir: string): (arg: string) => string {
    return (arg: string) => {
        ...
        const outDirW32 = path.normalize(expandPlaceHolders(configuration.get('dev.outDir') as string))
