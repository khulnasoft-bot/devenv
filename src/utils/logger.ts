...
const LOG_PANEL = vscode.window.createOutputChannel('Dev Workshop', 'dev_workshop_log')
const COMPILER_PANEL = vscode.window.createOutputChannel('Dev Compiler')
...
function initStatusBarItem() {
    STATUS_ITEM.command = 'dev-workshop.actions'
    STATUS_ITEM.show()
...
function showErrorMessageWithExtensionLogButton(message: string) {
    const res = showErrorMessage(message, 'Open Dev Workshop log')
    if (res) {
        return res.then(option => {
            switch (option) {
                case 'Open Dev Workshop log': {
                    showLog()
                    break
                }
                default: {
