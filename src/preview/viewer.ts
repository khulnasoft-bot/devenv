...
import type { ClientRequest, PdfViewerParams, PdfViewerState } from '../../types/dev-workshop-protocol-types/index'
...
async function viewInCustomEditor(pdfFile: string): Promise<void> {
    ...
    if (editorGroup === 'left') {
        ...
        if (currentColumn && currentColumn > 1) {
            ...
            await vscode.commands.executeCommand('vscode.openWith', pdfUri, 'dev-workshop-pdf-hook', showOptions)
...
async function viewInWebviewPanel(pdfUri: vscode.Uri, tabEditorGroup: string, preserveFocus: boolean): Promise<void> {
    ...
    const webviewPanel = vscode.window.createWebviewPanel('dev-workshop-pdf', path.basename(pdfUri.path), {
        viewColumn: vscode.ViewColumn.Active,
