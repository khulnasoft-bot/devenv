...
async function clean(rootFile?: string): Promise<void> {
    ...
    const cleanMethod = configuration.get('dev.clean.method') as string
    switch (cleanMethod) {
        case 'glob':
            return cleanGlob(rootFile)
        case 'cleanCommand':
            await configuration.update('dev.clean.method', 'command')
...
async function cleanGlob(rootFile: string): Promise<void> {
    const configuration = vscode.workspace.getConfiguration('dev-workshop', vscode.Uri.file(rootFile))
    const globPrefix = (configuration.get('dev.clean.subfolder.enabled') as boolean) ? './**/' : ''
    const globs = (configuration.get('dev.clean.fileTypes') as string[])
...
function cleanCommand(rootFile: string): Promise<void> {
    const configuration = vscode.workspace.getConfiguration('dev-workshop', vscode.Uri.file(rootFile))
    const command = configuration.get('dev.clean.command') as string
    const args = (configuration.get('dev.clean.args') as string[])
    ...
    return new Promise((resolve, _reject) => {
        // issue #3679 #3687: spawning with `detached: true` causes devmk from MiKTeX to fail on Windows when "install on-the-fly" is enabled
