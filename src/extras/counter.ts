...
function runTeXCount(file: string, merge: boolean = true): Promise<boolean> {
    ...
    proc.on('error', err => {
        ...
        void logger.showErrorMessage('TeXCount failed. Please refer to Dev Workshop Output for details.')
    })
        ...
        proc.on('exit', exitCode => {
            if (exitCode !== 0) {
                ...
                if (words) {
                    ...
                    state.texCountMessage = `There are ${words[1]} words ${floatMsg}in the ${merge ? 'Dev project' : 'opened Dev file'}.`
                    resolve(true)
                }
            }
        })
