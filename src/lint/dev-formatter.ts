...
async function formatDocument(document: vscode.TextDocument, range?: vscode.Range): Promise<vscode.TextEdit[]> {
    ...
    const pathMeta = configuration.get('devindent.path') as string
    formatterArgs = configuration.get('devindent.args') as string[]
    logger.log('Start formatting with devindent.')
    try {
        if (formatter === '') {
            formatter = pathMeta
            const devindentPresent = await checkPath()
            if (!devindentPresent) {
                formatter = ''
                logger.log(`Can not find ${formatter} in PATH: ${process.env.PATH}`)
                void logger.showErrorMessage('Can not find devindent in PATH.')
                return []
            }
        }
...
function checkPath(): Thenable<boolean> {
    ...
    if (useDocker) {
        ...
        if (process.platform === 'win32') {
            formatter = path.resolve(lw.extensionRoot, './scripts/devindent.bat')
        } else {
            formatter = path.resolve(lw.extensionRoot, './scripts/devindent')
    ...
    if (path.isAbsolute(formatter)) {
        if (fs.existsSync(formatter)) {
            ...
            logger.log(`The path of devindent is absolute and not found: ${formatter}`)
    ...
    const checkFormatter = (resolve: (value: boolean) => void, isFirstTry: boolean = true) => {
        ...
        check.on('close', code => {
            if (code && isFirstTry) {
                logger.log(`Error when checking devindent: ${stderr}`)
                formatter += fileExt
                logger.log(`Checking devindent: ${checker} ${formatter}`)
                checkFormatter(resolve, false)
            } else if (code) {
                logger.log(`Error when checking devindent: ${stderr}`)
                resolve(false)
            } else {
                logger.log(`Checking devindent is ok: ${stdout}`)
                resolve(true)
            }
        })
    }
...
function format(document: vscode.TextDocument, range?: vscode.Range): Thenable<vscode.TextEdit[]> {
    return new Promise((resolve, _reject) => {
        ...
        // The version of devindent shipped with current dev distributions doesn't support piping in the data using stdin, support was
        // only added on 2018-01-13 with version 3.4 so we have to create a temporary file
        const textToFormat = document.getText(range)
        const temporaryFile = documentDirectory + path.sep + '__devindent_temp_' + path.basename(document.fileName)
        ...
        logger.logCommand('Formatting Dev.', formatter, args)
        ...
        worker.on('error', err => {
            ...
            void logger.showErrorMessage('Formatting failed. Please refer to Dev Workshop Output for details.')
...
class FormattingProvider implements vscode.DocumentFormattingEditProvider, vscode.DocumentRangeFormattingEditProvider {
    constructor() {
        ...
        if (!currentOs) {
            logger.log('DevFormatter: Unsupported OS')
        }
        lw.onConfigChange('devindent.path', () => formatter = '')
    }
