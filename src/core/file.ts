 ...
 * TeX-related language identifiers. These identifiers include 'tex', 'dev',
 * 'dev-expl3', 'doctex', 'pweave', 'jlweave', and 'rsweave'.
 *
...
function hasTeXLangId(langId: string): boolean {
    return ['tex', 'dev', 'dev-expl3', 'doctex', 'pweave', 'jlweave', 'rsweave'].includes(langId)
}
...
/**
 * Determines the output directory for a given Dev file path.
 *
 * This function calculates the output directory where Dev compilation
 * artifacts will be stored. If a specific Dev file path is provided, the
 ...
 *
 * The process begins by checking if the provided Dev file path (`texPath`) is
 * defined; if not, it defaults to the root file path of the Dev workshop. If
 * both are undefined, it returns the current directory (`./`). The function
 * retrieves the configuration for the Dev workshop and extracts the output
 ...
 * trailing slashes.
 *
 * @param {string} [texPath] - The path to the Dev file. If not provided, the
...
function getOutDir(texPath?: string): string {
    ...
    const configuration = vscode.workspace.getConfiguration('dev-workshop', vscode.Uri.file(texPath))
    const outDir = configuration.get('dev.outDir') as string || './'
...
/**
 * Retrieves the job name for a Dev file based on the provided file path.
 *
 * If a job name is found in the configuration 'dev.jobname', it is returned;
 ...
 *
 * @param {string} texPath - The file path of the Dev document.
 * @returns {string} - The job name for the Dev document, either from the
 * configuration or derived from the file name.
 */
function getJobname(texPath: string): string {
    const jobname = vscode.workspace.getConfiguration('dev-workshop', vscode.Uri.file(texPath)).get('dev.jobname') as string
 ...
 * during the compilation of a Dev document. It starts by identifying the root
...
/**
 * A cache object for storing resolved paths of Dev targets.
 *
...
/**
 * Resolves the path to a given Dev target using the `kpsewhich` command.
 *
 * This function uses `kpsewhich` to find the path to a specified Dev target,
 ...
 *
 * @param {string} target - The Dev target to resolve, such as a file name.
...
function kpsewhich(target: string, isBib: boolean = false): string | undefined {
    ...
    const command = vscode.workspace.getConfiguration('dev-workshop').get('kpsewhich.path') as string
 ...
 *
 * This function first retrieves the configuration 'dev.bibDirs' to obtain
 ...
 * the bibliography file. Additionally, if the root directory of the Dev
...
function getBibPath(bib: string, baseDir: string): string[] {
    ...
    const bibDirs = configuration.get('dev.bibDirs') as string[]
