...
export function recipes(recipe?: string) {
    ...
    const candidates = configuration.get('dev.recipes') as {name: string}[]
    if (!candidates) {
        return
    }
    if (recipe) {
    ...
    return vscode.window.showQuickPick(candidates.map(candidate => candidate.name), {
        placeHolder: 'Please Select a Dev Recipe'
    }).then(selected => {
        if (!selected) {
            return
        }
...
export async function view(mode?: 'tab' | 'browser' | 'external' | vscode.Uri) {
    ...
    if (rootFile === undefined) {
        logger.log('Cannot find Dev root PDF to view.')
        return
    }
...
export function synctex() {
    ...
    if (lw.root.subfiles.path && configuration.get('dev.rootFile.useSubFile')) {
...
export async function clean(): Promise<void> {
    ...
    const rootFile = lw.root.file.path
    if (rootFile === undefined) {
        logger.log('Cannot find Dev root file to clean.')
        return
    }
    let pickedRootFile: string | undefined = rootFile
    if (lw.root.subfiles.path) {
...
export async function gotoSection(filePath: string, lineNumber: number) {
    ...
    if (vscode.window.activeTextEditor) {
        ...
        if (vscode.workspace.getConfiguration('dev-workshop').get('view.outline.sync.viewer') as boolean) {
...
export async function changeHostName() {
    ...
    const proceed = (await vscode.window.showInputBox({
        prompt: 'Changing Dev Workshop server hostname can expose your computer to the public and is under severe security risk. CORS is also disabled. Do you want to continue?',
    ...
    const hostname = await vscode.window.showInputBox({
        prompt: 'Please input the new hostname that Dev Workshop server will listen to. This change will be reset on closing VSCode.',
...
export function resetHostName() {
    ...
    void vscode.window.showInformationMessage('Dev Workshop server listening to 127.0.0.1 with CORS. You are safe now.')
}

export async function actions() {
    logger.log('ACTIONS command invoked.')
    return vscode.commands.executeCommand('workbench.view.extension.dev-workshop-activitybar').then(() => vscode.commands.executeCommand('workbench.action.focusActiveEditorGroup'))
}

/**
 * Insert the snippet with name name.
 * @param name  the name of a snippet contained in dev.json
 */
...
async function quickPickRootFile(rootFile: string, localRootFile: string, verb: string): Promise<string | undefined> {
    ...
    const doNotPrompt = configuration.get('dev.rootFile.doNotPrompt') as boolean
    if (doNotPrompt) {
        if (configuration.get('dev.rootFile.useSubFile')) {
