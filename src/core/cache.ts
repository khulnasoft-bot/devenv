 ...
 * to ignore from the 'dev.watch.files.ignore' configuration and uses the
...
function isExcluded(filePath: string): boolean {
    const globsToIgnore = vscode.workspace.getConfiguration('dev-workshop').get('dev.watch.files.ignore') as string[]
...
async function updateAST(fileCache: FileCache): Promise<void> {
    logger.log(`Parse Dev AST: ${fileCache.filePath} .`)
    const start = performance.now()
    fileCache.ast = await lw.parser.parse.tex(fileCache.content)
    const elapsed = performance.now() - start
    logger.log(`Parsed Dev AST in ${elapsed.toFixed(2)} ms: ${fileCache.filePath} .`)
}
 ...
 * path, root path, and additional Dev directories configured in the
...
async function updateChildrenXr(fileCache: FileCache, rootPath: string) {
    ...
    while (true) {
        ...
        const texDirs = vscode.workspace.getConfiguration('dev-workshop').get('dev.texDirs') as string[]
 ...
 * of a Dev document. It retrieves the path to the .fls file associated with
