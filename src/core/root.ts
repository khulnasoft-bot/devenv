...
/**
 * Finds the Dev project's root file.
 *
 * This function employs multiple strategies to find the root file for the Dev
    ...
    for (const method of findMethods) {
        ...
        if (rootFilePath === root.file.path) {
            ...
            void lw.cache.refreshCache(rootFilePath).then(async () => {
                ...
                // It happens a lot with subfiles, https://tex.stackexchange.com/questions/289450/path-of-figures-in-different-directories-with-subfile-dev
...
/**
 * Gets the indicator regex based on the Dev configuration.
 *
 * This function retrieves the indicator regex based on the Dev configuration.
...
function getIndicator(): RegExp {
    ...
    const indicator = configuration.get('dev.rootFile.indicator')
    switch (indicator) {
        case '\\documentclass[]{}':
...
async function findInWorkspace(): Promise<string | undefined> {
    ...
    const rootFilesIncludePatterns = configuration.get('dev.search.rootFiles.include') as string[]
    const rootFilesIncludeGlob = '{' + rootFilesIncludePatterns.join(',') + '}'
    const rootFilesExcludePatterns = configuration.get('dev.search.rootFiles.exclude') as string[]
