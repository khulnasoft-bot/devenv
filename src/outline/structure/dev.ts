...
const logger = lw.log('Structure', 'Dev')

type StructureConfig = {
    // The Dev macros to be extracted.
...
export async function construct(filePath: string | undefined = undefined, subFile: boolean = true): Promise<TeXElement[]> {
    ...
    const config = refreshDevModelConfig(subFile)
...
async function parseNode(
    ...
    if (node.type === 'macro' && config.macros.secs.includes(node.content) && node.args?.[2].openMark === '{') {
        ...
        // globbing arguments in unified-dev.
        element = {
...
function refreshDevModelConfig(subFile: boolean = true, defaultFloats = ['frame']): StructureConfig {
    const configuration = vscode.workspace.getConfiguration('dev-workshop')
    ...
    const structConfig: StructureConfig = {
        ...
        secIndex: {},
        texDirs: configuration.get('dev.texDirs') as string[],
        subFile,
