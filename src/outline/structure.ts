...
import { construct as constructDev } from './structure/dev'
...
function reveal(e: vscode.TextEditorSelectionChangeEvent) {
    if (!(vscode.workspace.getConfiguration('dev-workshop').get('view.outline.follow.editor') as boolean) || !state.view.visible) {
        return
    }
...
/**
 * Return the dev or bibtex structure
 *
...
async function build(force: boolean): Promise<TeXElement[]> {
    ...
    if (document?.languageId === 'doctex') {
        ...
        if (force || !state.cachedTeX) {
            ...
            state.cachedTeX = await constructDev()
...
class StructureProvider implements vscode.TreeDataProvider<TeXElement> {
    ...
    getTreeItem(element: TeXElement): vscode.TreeItem {
        ...
        treeItem.command = {
            command: 'dev-workshop.goto-section',
            title: '',
...
const state = {
    ...
    view: vscode.window.createTreeView('dev-workshop-structure', { treeDataProvider, showCollapseAll: true }),
    treeDataProvider
}
