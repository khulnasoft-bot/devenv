 ...
 * @param {string | undefined} rootFile - Path to the root Dev file.
...
function getStepString(step: Step): string {
    ...
    if(step.rootFile) {
        ...
        const configuration = vscode.workspace.getConfiguration('dev-workshop', rootFileUri)
        const showFilename = configuration.get<boolean>('dev.build.rootfileInStatus', false)
        if(showFilename) {
