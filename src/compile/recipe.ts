...
setDockerImage()
lw.onConfigChange('docker.image.dev', setDockerImage)
function setDockerImage() {
    const dockerImageName: string = vscode.workspace.getConfiguration('dev-workshop').get('docker.image.dev', '')
    logger.log(`Set $DEVENV_DOCKER_DEV: ${JSON.stringify(dockerImageName)}`)
    process.env['DEVENV_DOCKER_DEV'] = dockerImageName
}

setDockerPath()
lw.onConfigChange('docker.path', setDockerPath)
function setDockerPath() {
    const dockerPath: string = vscode.workspace.getConfiguration('dev-workshop').get('docker.path', '')
    logger.log(`Set $DEVENV_DOCKER_PATH: ${JSON.stringify(dockerPath)}`)
    process.env['DEVENV_DOCKER_PATH'] = dockerPath
}

/**
 * Build Dev project using the recipe system. Creates Tools containing the
 * tool info and adds them to the queue. Initiates a buildLoop if there is no
 * running one.
 *
 * @param {string} rootFile - Path to the root Dev file.
...
export async function build(rootFile: string, langId: string, buildLoop: () => Promise<void>, recipeName?: string) {
    ...
    if (tools?.map(tool => tool.command).includes('devmk') && rootFile === lw.root.subfiles.path && lw.root.file.path) {
 ...
 * Dev macros do not create the output directory themselves.
 *
 * @param {string} rootFile - Path to the root Dev file.
 */
...
async function createBuildTools(rootFile: string, langId: string, recipeName?: string): Promise<Tool[] | undefined> {
    ...
    const configuration = vscode.workspace.getConfiguration('dev-workshop', vscode.Uri.file(rootFile))
    const magic = await findMagicComments(rootFile)

    if (recipeName === undefined && magic.tex && !configuration.get('dev.build.forceRecipeUsage')) {
        ...
        const tools = configuration.get('dev.tools') as Tool[]
...
function createBuildMagic(rootFile: string, magicTex: Tool, magicBib?: Tool): Tool[] {
    ...
    if (!magicTex.args) {
        magicTex.args = configuration.get('dev.magic.args') as string[]
        magicTex.name = lw.constant.TEX_MAGIC_PROGRAM_NAME + lw.constant.MAGIC_PROGRAM_ARGS_SUFFIX
    }
    if (magicBib) {
        if (!magicBib.args) {
            magicBib.args = configuration.get('dev.magic.bib.args') as string[]
...
function findRecipe(rootFile: string, langId: string, recipeName?: string): Recipe | undefined {
    ...
    const recipes = configuration.get('dev.recipes') as Recipe[]
    const defaultRecipeName = configuration.get('dev.recipe.default') as string
...
function populateTools(rootFile: string, buildTools: Tool[]): Tool[] {
    ...
    buildTools.forEach(tool => {
            ...
            switch (tool.command) {
                case 'devmk':
                    ...
                    if (process.platform === 'win32') {
                        tool.command = path.resolve(lw.extensionRoot, './scripts/devmk.bat')
                    } else {
                        tool.command = path.resolve(lw.extensionRoot, './scripts/devmk')
        ...
        if (configuration.get('dev.option.maxPrintLine.enabled')) {
            tool.args = tool.args ?? []
            const isLuaDev = tool.args.includes('-luadev') ||
                               tool.args.includes('-pdflua') ||
                               tool.args.includes('-pdfluadev') ||
                               tool.args.includes('--luadev') ||
                               tool.args.includes('--pdflua') ||
                               tool.args.includes('--pdfluadev')
            if (isMikTeX() && ((tool.command === 'devmk' && !isLuaDev) || tool.command === 'pdfdev')) {
...
/**
 * Check whether the Dev toolchain compilers are provided by MikTeX.
 *
 * @returns {boolean} - True if the Dev toolchain is provided by MikTeX;
 * otherwise, false.
 */
function isMikTeX(): boolean {
    if (state.isMikTeX === undefined) {
        try {
            if (lw.external.sync('pdfdev --version').toString().match(/MiKTeX/)) {
                state.isMikTeX = true
                logger.log('`pdfdev` is provided by MiKTeX.')
            } else {
                state.isMikTeX = false
            }
        } catch (e) {
            logger.log('Cannot run `pdfdev` to determine if we are using MiKTeX.')
            state.isMikTeX = false
        }
    }
