...
function autoBuild(file: string, type: 'onFileChange' | 'onSave', bibChanged: boolean = false) {
    const configuration = vscode.workspace.getConfiguration('dev-workshop', vscode.Uri.file(file))
    if (configuration.get('dev.autoBuild.run') as string !== type) {
        return
    }
    ...
    if (!bibChanged && lw.root.subfiles.path && configuration.get('dev.rootFile.useSubFile')) {
 ...
 * Two conditions are considered: the presence of `dev.autoBuild.interval`
...
function canAutoBuild(): boolean {
    const configuration = vscode.workspace.getConfiguration('dev-workshop', lw.root.file.path ? vscode.Uri.file(lw.root.file.path) : undefined)
    return Date.now() - lw.compile.lastAutoBuildTime >= (configuration.get('dev.autoBuild.interval', 1000) as number)
}

let isBuilding = false
/**
 * Initiates the build process for the Dev project. It can build the entire
 ...
 * @param {string | undefined} rootFile - The path of the Dev root file.
...
async function build(skipSelection: boolean = false, rootFile: string | undefined = undefined, languageId: string | undefined = undefined, recipe: string | undefined = undefined) {
    ...
    const configuration = vscode.workspace.getConfiguration('dev-workshop', workspace)
    const externalBuildCommand = configuration.get('dev.external.build.command') as string
    const externalBuildArgs = configuration.get('dev.external.build.args') as string[]
    ...
    if (rootFile === undefined || languageId === undefined) {
        logger.log('Cannot find Dev root file. See https://github.com/khulnasoft/devenv/wiki/Compile#the-root-file')
        return
    }
 ...
 * possibly a sub-file. If in such a case, the compile command is `devmk`, the
...
function spawnProcess(step: Step): ProcessEnv {
    const configuration = vscode.workspace.getConfiguration('dev-workshop', step.rootFile ? vscode.Uri.file(step.rootFile) : undefined)
    if (step.index === 0 || configuration.get('dev.build.clearLog.everyRecipeStep.enabled') as boolean) {
    ...
    if (!step.isExternal &&
        ...
        if (step.command === 'devmk' && step.rootFile === lw.root.subfiles.path && lw.root.dir.path) {
    ...
    logger.log(`Dev build process spawned with PID ${lw.compile.process.pid}.`)
    return env
}
...
function handleProcessError(env: ProcessEnv, stderr: string, err: Error) {
    logger.logError(`Dev fatal error on PID ${lw.compile.process?.pid}.`, err)
...
function handleExitCodeError(step: Step, env: ProcessEnv, stderr: string, code: number | null, signal: NodeJS.Signals | null) {
    ...
    const configuration = vscode.workspace.getConfiguration('dev-workshop', step.rootFile ? vscode.Uri.file(step.rootFile) : undefined)
    if (!step.isExternal && signal !== 'SIGTERM' && !step.isRetry && configuration.get('dev.autoBuild.cleanAndRetry.enabled')) {
...
function handleNoRetryError(configuration: vscode.WorkspaceConfiguration, step: RecipeStep) {
    ...
    if (['onFailed', 'onBuilt'].includes(configuration.get('dev.autoClean.run') as string)) {
        void lw.extra.clean(step.rootFile).then(() => lw.event.fire(lw.event.AutoCleaned))
    }
...
async function afterSuccessfulBuilt(lastStep: Step, skipped: boolean) {
    ...
    const configuration = vscode.workspace.getConfiguration('dev-workshop', vscode.Uri.file(lastStep.rootFile))
    ...
    if (['onSucceeded', 'onBuilt'].includes(configuration.get('dev.autoClean.run') as string)) {
