import type { ClientRequest, PanelRequest, ServerResponse } from '../../types/dev-workshop-protocol-types/index'
...
async function reconnect() {
    ...
    console.log('Closed: WebSocket to Dev Workshop.')

    // Since WebSockets are disconnected when PC resumes from sleep,
    // we have to reconnect. https://github.com/khulnasoft/devenv/pull/1812
    await utils.sleep(3000)

    let tries = 1
    while (tries <= 10) {
        console.log(`Try to reconnect to Dev Workshop: (${tries}/10).`)
        try {
            initConnect()
            const ws = await websocket
            if (ws.readyState !== 1) {
            ...
            console.log('Reconnected: WebSocket to Dev Workshop.')
            return
        } catch (e) {
            console.error(e)
        }
    ...
    console.error('Cannot reconnect to Dev Workshop.')
}
