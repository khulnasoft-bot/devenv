...
export const set = {
    ...
    config: async (section: string, value: any) => {
        await vscode.workspace.getConfiguration('dev-workshop').update(section, value)
...
class TextDocument implements vscode.TextDocument {
    ...
    constructor(filePath: string, content: string, { languageId = 'dev', isDirty = false, isClosed = false, scheme = 'file' }: { languageId?: string, isDirty?: boolean, isClosed?: boolean, scheme?: string }) {
