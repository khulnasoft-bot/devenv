...
describe(path.basename(__filename).split('.')[0] + ':', () => {
    ...
    describe('lw.file.getOutDir', () => {
        ...
        it('should get output directory without root or input dev', () => {
            assert.pathStrictEqual(lw.file.getOutDir(), './')
        })

        it('should get output directory with an input dev', () => {
        ...
        it('should get output directory with an input dev over the root', () => {
            const rootDir = get.path(fixture)
            const texPath = get.path(fixture, 'main.tex')
        ...
        it('should get output directory with absolute `dev.outDir` and root', async () => {
            await set.config('dev.outDir', '/output')
            set.root(fixture, 'main.tex')
            assert.pathStrictEqual(lw.file.getOutDir(), '/output')
        })

        it('should get output directory with relative `dev.outDir` and root', async () => {
            await set.config('dev.outDir', 'output')
            set.root(fixture, 'main.tex')
            assert.pathStrictEqual(lw.file.getOutDir(), 'output')
        })

        it('should get output directory with relative `dev.outDir` with leading `./` and root', async () => {
            await set.config('dev.outDir', './output')
            set.root(fixture, 'main.tex')
            assert.pathStrictEqual(lw.file.getOutDir(), 'output')
        })

        it('should get output directory with relative `dev.outDir`, root, and an input dev', async () => {
            const texPath = get.path(fixture, 'main.tex')
        ...
        it('should get output directory with placeholder in `dev.outDir` and root', async () => {
            await set.config('dev.outDir', '%DIR%')
            set.root(fixture, 'main.tex')
            assert.pathStrictEqual(lw.file.getOutDir(), lw.root.dir.path)
        })

        it('should get output directory with placeholder in `dev.outDir`, root, and an input dev', async () => {
            const rootDir = get.path(fixture)
            const texPath = get.path(fixture, 'main.tex')
        ...
        it('should get output directory from last compilation if `dev.outDir` is `%DIR%`', async () => {
            await set.config('dev.outDir', '%DIR%')
            set.root(fixture, 'main.tex')
        ...
        it('should ignore output directory from last compilation if `dev.outDir` is not `%DIR%`', async () => {
            await set.config('dev.outDir', '/output')
            set.root(fixture, 'main.tex')
        ...
        it('should handle empty `dev.outDir` correctly', async () => {
            await set.config('dev.outDir', '')
            set.root(fixture, 'main.tex')
            assert.pathStrictEqual(lw.file.getOutDir(), './')
        })

        it('should handle absolute `dev.outDir` with trailing slashes correctly', async () => {
            await set.config('dev.outDir', '/output/')
            set.root(fixture, 'main.tex')
            assert.pathStrictEqual(lw.file.getOutDir(), '/output')
        })

        it('should handle relative `dev.outDir` with trailing slashes correctly', async () => {
            await set.config('dev.outDir', 'output/')
            set.root(fixture, 'main.tex')
            assert.pathStrictEqual(lw.file.getOutDir(), 'output')
        })
    ...
    describe('lw.file.getBibPath', () => {
        ...
        it('should correctly find BibTeX files in `dev.bibDirs`', async () => {
            set.root(fixture, 'main.tex')
            await set.config('dev.bibDirs', [ path.resolve(lw.root.dir.path ?? '', 'subdir') ])
    ...
    describe('lw.file.getLangId', () => {
        it('should return "dev" for .tex files', () => {
            assert.strictEqual(lw.file.getLangId('example.tex'), 'dev')
        })
        ...
        it('should handle paths with folders correctly', () => {
            assert.strictEqual(lw.file.getLangId('folder/example.tex'), 'dev')
        })
    })
    ...
    describe('lw.file.getJobname', () => {
        it('should return the jobname if present in configuration', async () => {
            ...
            await set.config('dev.jobname', 'myJob')
        ...
        it('should return the name of the input texPath if configuration is not set', async () => {
            ...
            await set.config('dev.jobname', undefined) // Ensuring the jobname is not set
            const expectedJobname = path.parse(texPath).name
            assert.strictEqual(lw.file.getJobname(texPath), expectedJobname)
        })
    })
    ...
    describe('lw.file.hasTeXLangId', () => {
        it('should return true for supported TeX languages', () => {
            ...
            assert.ok(lw.file.hasTeXLangId('dev'))
            assert.ok(lw.file.hasTeXLangId('dev-expl3'))
    ...
    describe('lw.file.hasBibLangId', () => {
        ...
        it('should return false for non-BibTeX languages', () => {
            assert.ok(!lw.file.hasBibLangId('dev'))
    ...
    describe('lw.file.hasDtxLangId', () => {
        ...
        it('should return false for non-Doctex languages', () => {
            assert.ok(!lw.file.hasDtxLangId('dev'))
