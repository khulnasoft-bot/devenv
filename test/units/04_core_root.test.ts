...
describe(path.basename(__filename).split('.')[0] + ':', () => {
    ...
    describe('on root file deletion', () => {
        beforeEach(async () => {
            await set.config('dev.watch.delay', 100)
        })
    ...
    describe('lw.root.getIndicator', () => {
        it('should return \\documentclass indicator on selecting `\\documentclass[]{}`', async () => {
            await set.config('dev.rootFile.indicator', '\\documentclass[]{}')
        ...
        it('should return \\begin{document} indicator on selecting `\\begin{document}`', async () => {
            await set.config('dev.rootFile.indicator', '\\begin{document}')
            const indicator = lw.root._test.getIndicator()
        ...
        it('should return \\documentclass indicator on other values', async () => {
            await set.config('dev.rootFile.indicator', 'invalid value')
            const indicator = lw.root._test.getIndicator()
    ...
    describe('lw.root.findInWorkspace', () => {
        ...
        it('should follow `dev.search.rootFiles.include` config', async () => {
            await set.config('dev.search.rootFiles.include', [ 'absolutely-nothing.tex' ])
            const root = await lw.root._test.findInWorkspace()

            assert.strictEqual(root, undefined)
        })

        it('should follow `dev.search.rootFiles.exclude` config', async () => {
            await set.config('dev.search.rootFiles.exclude', [ '**/*' ])
            const root = await lw.root._test.findInWorkspace()
        ...
        it('should find the correct root from workspace', async () => {
            ...
            await set.config('dev.search.rootFiles.include', [ `${fixture}/find_workspace/**/*.tex` ])
            await set.config('dev.search.rootFiles.exclude', [ `${fixture}/find_workspace/**/parent.tex` ])
            const root = await lw.root._test.findInWorkspace()
        ...
        it('should ignore root file indicators in comments', async () => {
            await set.config('dev.search.rootFiles.include', [ `${fixture}/find_workspace/**/comment.tex` ])
            const root = await lw.root._test.findInWorkspace()
        ...
        it('should find the correct root if the children of root includes active editor', async () => {
            ...
            await set.config('dev.search.rootFiles.include', [ `${fixture}/find_workspace/**/*.tex` ])
            await set.config('dev.search.rootFiles.exclude', [ `${fixture}/find_workspace/main.tex` ])
