...
suite('Build TeX files test suite', () => {
    ...
    teardown(async () => {
        ...
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.tools', undefined)
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.outDir', undefined)
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.recipes', undefined)
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.build.forceRecipeUsage', undefined)
        ...
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.search.rootFiles.exclude', undefined)
    })
    ...
    test.run('same placeholders multiple times', async (fixture: string) => {
        const tools = [{name: 'devmk', command: 'devmk', args: ['-synctex=1', '-interaction=nonstopmode', '-file-line-error', '-pdf', '%DOC%', '%DOC%', '%DOC%']}]
    ...
    test.run('auto-detect subfile root and build 1', async (fixture: string) => {
        ...
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.rootFile.useSubFile', true)
        await test.load(fixture, [
    ...
    test.run('build with outDir', async (fixture: string) => {
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.outDir', './out')
        await test.load(fixture, [
            {src: 'base.tex', dst: 'main.tex'}
        ], {skipCache: true})
        await test.build(fixture, 'main.tex')
    ...
    test.run('basic build with spaces in outdir', async (fixture: string) => {
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.outDir', '%DIR%/out space')
        await test.load(fixture, [
            {src: 'base.tex', dst: 'main.tex'}
        ], {skipCache: true})
        await test.build(fixture, 'main.tex')
    ...
    test.run('build with !LW recipe', async (fixture: string) => {
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.build.forceRecipeUsage', true)
        const tools = [
            { name: 'touch', command: 'touch', args: ['fail.txt'], env: {} },
            { name: 'devmk', command: 'devmk', args: [ '-synctex=1', '-interaction=nonstopmode', '-file-line-error', '-pdf', '-outdir=%OUTDIR%', '%DOC%' ], env: {} }
        ]
        const recipes = [{name: 'touch', tools: ['touch']}, {name: 'devmk', tools: ['devmk']}]
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.tools', tools)
    ...
    test.run('build with invalid !LW recipe', async (fixture: string) => {
        ...
        const recipes = [{name: 'touch', tools: ['touch']}, {name: 'devmk_', tools: ['devmk']}]
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.tools', tools)
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.recipes', recipes)
        await test.load(fixture, [
            {src: 'magic_recipe.tex', dst: 'main.tex'}
    ...
    test.run('build main.tex with QuickPick', async (fixture: string) => {
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.rootFile.doNotPrompt', false)
        await test.load(fixture, [
            {src: 'subfile_base.tex', dst: 'main.tex'},
            {src: 'subfile_sub.tex', dst: 'sub/s.tex'}
        ], {local: 1, skipCache: true})
    ...
    test.run('test q/.../ with spaces in outdir on Windows', async (fixture: string) => {
        const tools = [{ name: 'devmk', command: 'devmk', args: ['-e', '$pdfdev=q/pdfdev %O -synctex=1 -interaction=nonstopmode -file-line-error %S/', '-outdir=%OUTDIR%', '-pdf', '%DOC%'], env: {} }]
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.tools', tools)
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.outDir', '%DIR%/out space')
        await test.load(fixture, [
            {src: 'base.tex', dst: 'main.tex'}
    ...
    test.run('test q/.../ with copy and remove on Windows', async (fixture: string) => {
        const tools = [
            { name: 'devmk', command: 'devmk', args: ['-e', '$pdfdev=q/pdfdev %O -synctex=1 -interaction=nonstopmode -file-line-error %S/', '-outdir=%OUTDIR%', '-pdf', '%DOC%'], env: {} },
        ...
        const recipes = [{name: 'devmk_copy', tools: ['devmk', 'copyPDF', 'removePDF']}]
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.tools', tools)
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.recipes', recipes)
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.outDir', '%DIR%/out space')
        await test.load(fixture, [
