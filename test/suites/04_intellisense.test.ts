...
suite('Intellisense test suite', () => {
    ...
    teardown(async () => {
        ...
        await vscode.workspace.getConfiguration('dev-workshop').update('intellisense.atSuggestion.trigger.dev', undefined)
        await vscode.workspace.getConfiguration('dev-workshop').update('intellisense.atSuggestion.user', undefined)
        await vscode.workspace.getConfiguration('dev-workshop').update('intellisense.citation.label', undefined)
        await vscode.workspace.getConfiguration('dev-workshop').update('intellisense.citation.format', undefined)
        await vscode.workspace.getConfiguration('dev-workshop').update('intellisense.label.command', undefined)
        await vscode.workspace.getConfiguration('dev-workshop').update('intellisense.argumentHint.enabled', undefined)
        await vscode.workspace.getConfiguration('dev-workshop').update('intellisense.command.user', undefined)
        await vscode.workspace.getConfiguration('dev-workshop').update('intellisense.package.exclude', undefined)
    })
    ...
    test.run('command intellisense with config `intellisense.argumentHint.enabled`', async (fixture: string) => {
        await vscode.workspace.getConfiguration('dev-workshop').update('intellisense.argumentHint.enabled', true)
        await test.load(fixture, [
            {src: 'intellisense/base.tex', dst: 'main.tex'},
            {src: 'intellisense/sub.tex', dst: 'sub/s.tex'}
        ])
        ...
        await vscode.workspace.getConfiguration('dev-workshop').update('intellisense.argumentHint.enabled', false)
        await test.load(fixture, [
            {src: 'intellisense/base.tex', dst: 'main.tex'},
            {src: 'intellisense/sub.tex', dst: 'sub/s.tex'}
        ])
    ...
    test.run('command intellisense with config `intellisense.command.user`', async (fixture: string) => {
        await vscode.workspace.getConfiguration('dev-workshop').update('intellisense.command.user', {'mycommand[]{}': 'notsamecommand[${2:option}]{$TM_SELECTED_TEXT$1}', 'parbox{}{}': 'defchanged', 'overline{}': ''})
        await test.load(fixture, [
            {src: 'intellisense/base.tex', dst: 'main.tex'},
            {src: 'intellisense/sub.tex', dst: 'sub/s.tex'}
        ])
    ...
    test.run('reference intellisense and config intellisense.label.command', async (fixture: string) => {
        ...
        await vscode.workspace.getConfiguration('dev-workshop').update('intellisense.label.command', ['label'])
        await test.load(fixture, [
            {src: 'intellisense/label.tex', dst: 'main.tex'}
        ])
    ...
    test.run('intellisense with config `intellisense.package.exclude`', async (fixture: string) => {
        await vscode.workspace.getConfiguration('dev-workshop').update('intellisense.package.exclude', ['lw-default', 'import', 'listings'])
        await test.load(fixture, [
            {src: 'intellisense/base.tex', dst: 'main.tex'},
            {src: 'intellisense/sub.tex', dst: 'sub/s.tex'}
        ])
        ...
        await vscode.workspace.getConfiguration('dev-workshop').update('intellisense.package.exclude', ['lw-default'])
        suggestions = test.suggest(0, 1)
        assert.ok(!suggestions.labels.includes('\\date{}'))
        assert.ok(suggestions.labels.includes('\\lstinline'))
        assert.ok(suggestions.labels.includes('\\lstinline'))

        await vscode.workspace.getConfiguration('dev-workshop').update('intellisense.package.exclude', ['import', 'listings'])
        suggestions = test.suggest(0, 1)
        assert.ok(suggestions.labels.includes('\\date{}'))
        assert.ok(!suggestions.labels.includes('\\lstinline'))
        assert.ok(!suggestions.labels.includes('\\lstinline'))
    ...
    test.run('citation intellisense and configs intellisense.citation.*', async (fixture: string) => {
        await vscode.workspace.getConfiguration('dev-workshop').update('intellisense.citation.label', 'bibtex key')
        await test.load(fixture, [
        ...
        await vscode.workspace.getConfiguration('dev-workshop').update('intellisense.citation.label', 'title')
        suggestions = test.suggest(2, 9)
        assert.strictEqual(suggestions.items.length, 3)
        assert.strictEqual(suggestions.items[0].label, 'A fake article')

        await vscode.workspace.getConfiguration('dev-workshop').update('intellisense.citation.label', 'authors')
        suggestions = test.suggest(2, 9)
        assert.strictEqual(suggestions.items.length, 3)
        assert.strictEqual(suggestions.items[0].label, 'Davis, J. and Jones, M.')

        await vscode.workspace.getConfiguration('dev-workshop').update('intellisense.citation.format', ['title', 'year', 'description', 'nonexisting'])
        suggestions = test.suggest(2, 9)
        assert.strictEqual(suggestions.items.length, 3)
        assert.ok(suggestions.items[0].filterText)
        assert.ok(!suggestions.items[0].filterText.includes('Journal of CI tests'))
    ...
    test.run('@-snippet intellisense and configs intellisense.atSuggestion*', async (fixture: string) => {
        ...
        await vscode.workspace.getConfiguration('dev-workshop').update('intellisense.atSuggestion.user', replaces)
        await test.load(fixture, [
            {src: 'intellisense/base.tex', dst: 'main.tex'},
            {src: 'intellisense/sub.tex', dst: 'sub/s.tex'}
        ])
        ...
        await vscode.workspace.getConfiguration('dev-workshop').update('intellisense.atSuggestion.trigger.dev', '#')
