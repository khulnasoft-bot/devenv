...
suite('Document structure test suite', () => {
    ...
    teardown(async () => {
        ...
        await vscode.workspace.getConfiguration('dev-workshop').update('view.outline.numbers.enabled', undefined)
        await vscode.workspace.getConfiguration('dev-workshop').update('view.outline.commands', undefined)
        await vscode.workspace.getConfiguration('dev-workshop').update('view.outline.sections', undefined)
        await vscode.workspace.getConfiguration('dev-workshop').update('view.outline.floats.enabled', undefined)
        await vscode.workspace.getConfiguration('dev-workshop').update('view.outline.floats.number.enabled', undefined)
        await vscode.workspace.getConfiguration('dev-workshop').update('view.outline.floats.caption.enabled', undefined)
    })
    ...
    test.run('section without numbering via `view.outline.numbers.enabled`', async (fixture: string) => {
        await vscode.workspace.getConfiguration('dev-workshop').update('view.outline.numbers.enabled', false)
        await test.load(fixture, [
            {src: 'structure/sections.tex', dst: 'main.tex'}
        ])
        const structure = await lw.outline.reconstruct()
    ...
    test.run('custom sections via `view.outline.sections`', async (fixture: string) => {
        await vscode.workspace.getConfiguration('dev-workshop').update('view.outline.sections', ['customsection', 'subsubsection'])
        await test.load(fixture, [
    ...
    test.run('custom section hierarchy via `view.outline.sections`', async (fixture: string) => {
        await vscode.workspace.getConfiguration('dev-workshop').update('view.outline.sections', ['section', 'subsection|subsubsection'])
        await test.load(fixture, [
            {src: 'structure/section_nest.tex', dst: 'main.tex'}
        ])
        const structure = await lw.outline.reconstruct()
    ...
    test.run('custom commands via `view.outline.commands`', async (fixture: string) => {
        await vscode.workspace.getConfiguration('dev-workshop').update('view.outline.commands', ['note'])
        await test.load(fixture, [
    ...
    test.run('float without numbering via `view.outline.floats.number.enabled`', async (fixture: string) => {
        await vscode.workspace.getConfiguration('dev-workshop').update('view.outline.floats.number.enabled', false)
        await test.load(fixture, [
            {src: 'structure/frames.tex', dst: 'main.tex'}
        ])
        const structure = await lw.outline.reconstruct()
    ...
    test.run('frames without title via `view.outline.floats.caption.enabled`', async (fixture: string) => {
        await vscode.workspace.getConfiguration('dev-workshop').update('view.outline.floats.caption.enabled', false)
        await test.load(fixture, [
            {src: 'structure/frame_title.tex', dst: 'main.tex'}
        ])
        const structure = await lw.outline.reconstruct()
    ...
    test.run('disable floats via `view.outline.floats.enabled`', async (fixture: string) => {
        await vscode.workspace.getConfiguration('dev-workshop').update('view.outline.floats.enabled', false)
        await test.load(fixture, [
            {src: 'structure/floats.tex', dst: 'main.tex'}
        ])
        const structure = await lw.outline.reconstruct()
