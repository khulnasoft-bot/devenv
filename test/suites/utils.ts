...
export function run(testName: string, cb: (fixturePath: string) => unknown, platforms?: NodeJS.Platform[], runonly?: boolean) {
    ...
    if (process.env['DEVENV_SUITE'] && !process.env['DEVENV_SUITE'].split(',').find(candidate => suite.name.includes(candidate))) {
        return
    }
    ...
    testIndex++
    const testFunction = (process.env['DEVENV_CITEST'] || !runonly) ? test : test.only
