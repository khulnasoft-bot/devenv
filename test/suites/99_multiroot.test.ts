...
suite('Multi-root workspace test suite', () => {
    ...
    teardown(async () => {
        ...
        const configs = [
            vscode.workspace.getConfiguration('dev-workshop'),
            vscode.workspace.getConfiguration('dev-workshop', vscode.workspace.workspaceFolders?.[0])
        ]
        const items = [
            'dev.tools',
            'dev.outDir',
            'dev.recipes',
            'dev.recipe.default',
            'dev.build.forceRecipeUsage',
            'dev.search.rootFiles.include',
            'dev.search.rootFiles.exclude',
            'dev.autoBuild.run',
            'dev.rootFile.doNotPrompt',
            'dev.rootFile.useSubFile',
    ...
    test.run('detect root with search.rootFiles.include', async (fixture: string) => {
        ...
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.search.rootFiles.include', ['**/alt/*.tex'])
    ...
    test.run('detect root with search.rootFiles.exclude', async (fixture: string) => {
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.rootFile.doNotPrompt', true)
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.search.rootFiles.exclude', ['*/*.tex'])
        await test.load(fixture, [
            {src: 'subfile_base.tex', dst: 'main.tex', ws: 'A'},
            {src: 'input_parentsub.tex', dst: 'alt/main.tex', ws: 'A'},
            {src: 'plain.tex', dst: 'sub/s.tex', ws: 'A'},
    ...
    test.run('basic build with default recipe name', async (fixture: string) => {
        const tools = [
            {name: 'devmk', command: 'devmk', args: ['-synctex=1', '-interaction=nonstopmode', '-file-line-error', '-pdf', '-outdir=%OUTDIR%', '-jobname=wsA', '%DOC%'], env: {}},
            {name: 'fake', command: 'touch', args: ['%DIR%/fake.pdf']}
        ]
        const recipes = [{name: 'devmk', tools: ['devmk']}, {name: 'fake', tools: ['fake']}]
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.tools', tools)
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.recipes', recipes)
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.recipe.default', 'fake')
        await vscode.workspace.getConfiguration('dev-workshop', vscode.workspace.workspaceFolders?.[0]).update('dev.recipe.default', 'devmk')
        await test.load(fixture, [
            {src: 'base.tex', dst: 'main.tex', ws: 'A'},
            {src: 'empty', dst: 'empty', ws: 'B'}
        ], {skipCache: true})
    ...
    test.run('basic build with unavailable lastUsed', async (fixture: string) => {
        ...
        const recipes = [{name: 'devmk A', tools: ['devmk']}, {name: 'fake', tools: ['fake']}]
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.tools', tools)
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.recipes', [])
        await vscode.workspace.getConfiguration('dev-workshop').update('dev.recipe.default', 'fake')
        await vscode.workspace.getConfiguration('dev-workshop', vscode.workspace.workspaceFolders?.[0]).update('dev.recipes', recipes)
        await vscode.workspace.getConfiguration('dev-workshop', vscode.workspace.workspaceFolders?.[0]).update('dev.recipe.default', 'lastUsed')
        await test.load(fixture, [
            {src: 'base.tex', dst: 'main.tex', ws: 'A'},
            {src: 'empty', dst: 'empty', ws: 'B'}
        ], {skipCache: true})
    ...
    test.run('basic build with outDir', async (fixture: string) => {
        await vscode.workspace.getConfiguration('dev-workshop', vscode.workspace.workspaceFolders?.[0]).update('dev.outDir', './out')
        await test.load(fixture, [
            {src: 'base.tex', dst: 'main.tex', ws: 'A'},
            {src: 'empty', dst: 'empty', ws: 'B'}
        ], {skipCache: true})
    ...
    test.run('build with forceRecipeUsage: true', async (fixture: string) => {
        ...
        await vscode.workspace.getConfiguration('dev-workshop', vscode.workspace.workspaceFolders?.[0]).update('dev.build.forceRecipeUsage', true)
        await test.load(fixture, [
    ...
    test.run('switching intellisense', async (fixture: string) => {
        ...
        await vscode.workspace.getConfiguration('dev-workshop', vscode.workspace.workspaceFolders?.[0]).update('intellisense.citation.label', 'title')
        await test.load(fixture, [
