...
suite('PDF viewer test suite', () => {
    ...
    suiteSetup(async () => {
        ...
        await vscode.workspace.getConfiguration('dev-workshop').update('view.pdf.viewer', 'tab')
    })

    teardown(async () => {
        await test.reset()

        await vscode.workspace.getConfiguration().update('dev-workshop.dev.outDir', undefined)
        await vscode.workspace.getConfiguration().update('dev-workshop.dev.rootFile.useSubFile', undefined)
        await vscode.workspace.getConfiguration().update('dev-workshop.dev.rootFile.doNotPrompt', undefined)
        await vscode.workspace.getConfiguration().update('dev-workshop.synctex.afterBuild.enabled', undefined)
    })
    ...
    test.run('view in singleton tab', async (fixture: string) => {
        await vscode.workspace.getConfiguration('dev-workshop').update('view.pdf.viewer', 'singleton')
        await test.load(fixture, [
            {src: 'base.tex', dst: 'main.tex'}
        ], {skipCache: true})
        ...
        await vscode.workspace.getConfiguration('dev-workshop').update('view.pdf.viewer', 'legacy')
        await lw.commands.view()
        await test.sleep(250)
    ...
    test.run('build main.tex and view it', async (fixture: string) => {
        await vscode.workspace.getConfiguration().update('dev-workshop.dev.rootFile.doNotPrompt', true)
        await vscode.workspace.getConfiguration().update('dev-workshop.dev.rootFile.useSubFile', false)
        await test.load(fixture, [
    ...
    test.run('build a subfile and view it', async (fixture: string) => {
        await vscode.workspace.getConfiguration().update('dev-workshop.dev.rootFile.doNotPrompt', true)
        await vscode.workspace.getConfiguration().update('dev-workshop.dev.rootFile.useSubFile', true)
        await test.load(fixture, [
            {src: 'subfile_base.tex', dst: 'main.tex'},
            {src: 'subfile_sub.tex', dst: 'sub/s.tex'}
        ], {local: 1, skipCache: true})
