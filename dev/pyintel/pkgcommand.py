    ...
    def parse_cwl_file(self, file_path: Union[Path, str], remove_spaces: bool = False) -> Pkg:
        ...
        for line in lines:
            ...
            if len(line) == 0:                      # empty line
                ...
                # https://github.com/khulnasoft/devenv/issues/3264#issuecomment-1138733921
                # Also include `(`, `)`, and `,`
                # https://github.com/khulnasoft/devenv/issues/4313#issuecomment-2214209089
                ...
                if ((re.match(r'\\left[^a-zA-Z]', line) is not None and '\\right' not in line) or
                    (re.match(r'\\right[^a-zA-Z]', line))): # Special cases in dev-document
                    continue
                match = re.match(r'\\([^[\{\n]*?)((?:\{|\[)[^#\n]*)?(#.*)?$', line)
                if match is None:
                    continue
